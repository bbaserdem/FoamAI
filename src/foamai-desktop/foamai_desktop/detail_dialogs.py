...
class MeshDetailDialog(QDialog):
    ...
    def populate_data(self):
        ...
        if self.mesh_data.generated_by_ai:
            self.ai_generated_label.setText("✅ Generated by AI - You can edit these values")
    ...
    def validate_input(self) -> bool:
        ...
        if not description and not content:
            QMessageBox.warning(self, "Validation Error", "Please provide either a description or file content.")
            return False
...
class SolverDetailDialog(QDialog):
    ...
    def validate_input(self) -> bool:
        ...
        if not selected_items:
            QMessageBox.warning(self, "Validation Error", "Please select a solver.")
            return False
        ...
        for i in range(self.params_layout.count()):
            ...
            if widget and hasattr(widget, 'layout'):
                ...
                if layout.count() >= 2:
                    ...
                    if name_edit and name_edit.text().strip():
                        has_parameters = True
                        break
        
        if not description and not content and not has_parameters:
            QMessageBox.warning(self, "Validation Error", "Please provide a description, content, or at least one parameter.")
            return False
